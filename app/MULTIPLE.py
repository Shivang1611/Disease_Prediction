...
# Update the hospital suggestion button handlers in each prediction section
        ...
        if any(input_vector[0]):
            try:
                ...
                if hasattr(model, 'predict'):
                    ...
                    # Show hospital suggestion button after prediction
                    st.markdown("---")
                    st.markdown("### Need help finding medical care?")
                    hospital_button = st.button("🏥 Find Nearby Hospitals", key="symptom_hospitals")
                    if hospital_button:
                        suggest_hospitals()    
